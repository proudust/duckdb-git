...
enum GitTagScanner {
    #[cfg(feature = "libgit-backend")]
    Libgit(libgit::LibGitTagScanner),
    #[cfg(feature = "gix-backend")]
    Gix(gix::GixTagScanner),
}
    ...
    fn open(
        ...
        match kind {
            BackendKind::Libgit => {
                #[cfg(feature = "libgit-backend")]
                {
                    Ok(Self::Libgit(libgit::LibGitTagScanner::open(
                        repo_path,
                        params,
                        column_indices,
                    )?))
            ...
            #[cfg(feature = "gix-backend")]
            BackendKind::Gix => Ok(Self::Gix(gix::GixTagScanner::open(
                repo_path,
                params,
                column_indices,
            )?)),
...
struct GitTagInitData {
    scanner: GitTagScanner,
    column_indices: Vec<u64>,
}

struct GitTagVTab;

impl VTab for GitTagVTab {
    type InitData = GitTagInitData;
    type BindData = GitTagParameter;
    ...
    fn init(info: &InitInfo) -> Result<Self::InitData, Box<dyn std::error::Error>> {
        let params = info.get_bind_data::<GitTagParameter>();
        let params = unsafe { &*params };
        let column_indices = info.get_column_indices();
        let repo_path = resolve_repo_path(&params.repo_path, params.backend)?;
        let scanner = GitTagScanner::open(&repo_path, params.backend, params, &column_indices)?;
...
pub fn register(con: &Connection) -> Result<(), Box<dyn Error>> {
    con.register_table_function::<GitTagVTab>("git_tag")?;
    Ok(())
}
