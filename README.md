...
[^3]: The repository is cached as a bare clone in the OS temp directory (`$TMPDIR/duckdb-git/...`) and refreshed on each query. Each backend keeps its own cache directory. Concurrent queries for the same URL — including from separate DuckDB processes — are serialized by an advisory lock on `$TMPDIR/duckdb-git/<backend>/<cache-dir>.lock`, so one waits while the other fetches or clones. Only anonymous HTTP(S) is supported — URLs with embedded credentials (`https://user:token@...`) are rejected. Do not put secrets in the URL; the cache may be readable by other users on shared machines.
